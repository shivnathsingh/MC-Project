        ...
        mAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            ...
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful())
                {
                    ...
                    curr_user = email;
                    curr_user = curr_user.replaceAll("[@.]", "");
                    FirebaseDatabase db = FirebaseDatabase.getInstance();
                    DatabaseReference ref = db.getReference("user");

                    ref.child(curr_user).setValue("");
                    // add to the user super coins
                    ref.child(curr_user).child("supercoins").setValue(1);
                    Toast.makeText(MainActivityNew.this, ""+curr_user, Toast.LENGTH_LONG).show();
